...
public class TimingUtil {
    ...
    public static <S extends State<S>> Trajectory<TimedState<S>> timeParameterizeTrajectory(
        ...
        successor.minAcceleration = maxDecceleration;
        ...
        for (int i = states.size() - 1; i >= 0; --i) {
            ...
            if (i >= states.size() - slowdownChunks) constraintState.minAcceleration = -maxDecceleration;
        ...
        double v = 0.0;
        for (int i = 0; i < states.size(); ++i) {
            final ConstrainedState constrainedState = constraintStates.get(i);
            // Advance t.
